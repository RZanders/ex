...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS datasheet (id INTEGER PRIMARY KEY, type WARCHAR, model WARCHAR, price FLOAT)")
...
def Create():
    if  TYPE.get() == "" or MODEL.get() == "" or PRICE.get() == "":
        ...
        Database()
        cursor.execute("INSERT INTO datasheet (type, model, price) VALUES(?, ?, ?)", (str(TYPE.get()), str(MODEL.get()), str(PRICE.get())
        conn.commit()
        TYPE.set("")
        MODEL.set("")
        PRICE.set("")
